

Uppdatering/Update 1.5.0
- Lagt till möjligheten att trycka på skärmen för att försöka igen, utan att behöva trycka på Försök igen. / Added the possibility to try again without pressing "Try Again".
- Lagt till ljudeffekter när man svänger, kraschar eller klarar banan. / Added sound effect for turns, crashes or completing the track.
- Lagt till möjligheten att hoppa över att se ban-sträckningen. / Added the possiblity to skip the preview of the track.
-------------------------------
Updatering/Update 1.4.0
Feature:
- Lagt till knapp för att byta mellan svenska och engelska. / Added a button to change between Swedish and English.
-------------------------------
Updatering/Update 1.3.0
Feature:
- Tagit bort hörn-cirklarna på Svår och Brutal nivå. / Removed the circles in the corners on difficulties "Hard and "Brutal".
-------------------------------
Updatering/Update 1.2.0
Feature:
- Lagt till att man ser bansträckningen innan man startar. / Added the feature to see the track layout before playing.
-------------------------------
Updatering/Update 1.1.0
Feature:
- Lagt till möjligheten att se ens framsteg i realtid medans man spelar en bana. / Added the function to see the progress in real time while playing a track.
- Lagt till en framstegs indikator som visuellt visar hur långt man kommit på banan. / Added a progressbar to visually display the progress on the track.
-------------------------------
Updatering/Update 1.0.1
Buggfix
- Tagit bort kravet att trycka i slutet för att gå i mål. / Removed the requirement to click at the end of the course to finish the track.
